...
const AddressComponent = () => {
  ...
  const fetchOrderAddress = async (addressId) => {
    ...
    try {
      ...
      if (error.response) {
        toast.error(
          `Failed to update order address: ${error.response.data.message}`
        );
      } else {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          marginTop: "25vh",
          borderTop: "1px solid var(--scroll-bar-color)",
          paddingTop: "10px",
        }}
      >
        <button
        ...
        <button onClick={handleProceedToBuy} className="pay-now-button">
          Proceed to Buy
        </button>
      </div>
