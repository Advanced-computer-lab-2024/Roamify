...
const TouristActivities = () => {
    ...
    useEffect(() => {
        const fetchActivities = async () => {
            try {
                const response = await axios.get("http://localhost:3000/api/get-activities");
    ...
    // Filter upcoming activities based on the current date
    ...
    const filteredUpcomingActivities = upcomingActivities.filter(activity => {
        const priceWithinRange = activity.price >= minPrice && activity.price <= maxPrice;
        const matchesDate = selectedDate ? new Date(activity.date).toISOString().split('T')[0] === selectedDate : true;
        const matchesCategory = selectedCategory ? activity.category?.name === selectedCategory : true;
        const matchesRating = selectedRating ? activity.rating === parseFloat(selectedRating) : true;

        return priceWithinRange && matchesDate && matchesCategory && matchesRating;
    });

    // Filter based on search type (name, category, or tags)
    const searchedActivities = filteredUpcomingActivities.filter(activity => {
        ...
        if (searchType === 'name') {
            ...
            return activity.tags && activity.tags.some(tag => tag.name.toLowerCase().includes(searchLower));
        }
        return true; // If no search type is selected, return all
    });

    // Select activities to display (filtered upcoming or all activities)
    const activitiesToDisplay = filterUpcoming ? searchedActivities : activities;
        ...
        <div className="activity-list-page flex flex-col items-center p-5 bg-gray-100 min-h-screen">
            ...
            <div className="flex flex-col w-full max-w-3xl mb-4">
                ...
                <div className="sort-controls flex justify-between w-full">
                    <button
                        className={`p-2 border rounded-lg ${!filterUpcoming ? 'bg-blue-500 text-white' : ''}`}
