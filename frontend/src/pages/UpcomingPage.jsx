...
import "../css/UpcomingPage.css"; // Add your custom styles here
...
const ItineraryPage = () => {
  ...
  const [searchType, setSearchType] = useState("name"); // State for search type (name, category, preferences)
  ...
  // Search filter logic based on the selected search type
  const filteredItineraries = safeItineraries.filter((itinerary) => {
    const searchLower = searchTerm.toLowerCase();

    if (searchType === "name") {
      //   return itinerary.name.toLowerCase().includes(searchLower);
    } else if (searchType === "category") {
      return itinerary.category?.toLowerCase().includes(searchLower);
    } else if (searchType === "preferences") {
      return itinerary.preferenceTags?.some((pref) =>
  ...
  const sortedItineraries = filteredItineraries
    .filter((itinerary) => {
      const priceWithinRange =
        itinerary.price >= minBudget && itinerary.price <= maxBudget;
      const matchesDate = selectedDate
        ? new Date(itinerary.date) >= new Date(selectedDate)
        : true;
      ...
      return (
        priceWithinRange && matchesDate && matchesPreferences && matchesLanguage
      );
    })
    .sort((a, b) => {
        ...
        <ul>
          {sortedItineraries.map((itinerary) => (
            <li key={itinerary.id}>
              ...
              <p>
                Destinations: {itinerary.destinations?.join(", ") || "None"}
              </p>
