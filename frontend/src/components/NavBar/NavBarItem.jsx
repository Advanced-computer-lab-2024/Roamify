...
const NavBarItem = ({ userType, icon, name, children, route }) => {
        ...
        <>
          ...
          >
            ...
            <ArrowDownIcon
              ...
              width="30"
              fill="white"
              className={`ml-auto transform transition-transform duration-300 ${isOpen ? "rotate-180" : "rotate-270"
                }`}
            />
          </button>
          {isOpen && (
            <ul className="sub-menu transition-all duration-300 ease-in-out max-h-64">
              {children.map((child, index) => (
                <Link
                  key={index}
                  to={`/${userType}/${name.toLowerCase()}/${child.name
                    .toLowerCase()
