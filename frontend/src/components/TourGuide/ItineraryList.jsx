...
const ItineraryList = () => {
    ...
    const fetchItineraries = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/tourguide/get-my-itinearies/${profileId}`
        );
  ...
  const handleDelete = async (itineraryId) => {
    try {
      await axios.delete(
        `http://localhost:3000/tourguide/delete-itineary/${profileId}/${itineraryId}`
      );
      setItineraries((prev) =>
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (selectedItinerary) {
        await axios.put(
          `http://localhost:3000/tourguide/update-itineary/${profileId}/${selectedItinerary._id}`,
          selectedItinerary
        );
        ...
        const response = await axios.get(
          `http://localhost:3000/tourguide/get-my-itineraries/${profileId}`
        );
        setItineraries(response.data);
      }
    } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    // Handle array fields
    if (
      name.startsWith("availableDates") ||
      name.startsWith("locations") ||
      name.startsWith("preferenceTags")
    ) {
              ...
              >
                <div>
                  ...
                  {itinerary.preferenceTags && (
                    <p>
                      Preference Tags: {itinerary.preferenceTags.join(", ")}
                    </p>
                  )}
                </div>
                <div className="flex gap-2">
                  ...
                  <DeleteButton onClick={() => handleDelete(itinerary._id)}>
                    Delete
                  </DeleteButton>
                </div>
              </li>
