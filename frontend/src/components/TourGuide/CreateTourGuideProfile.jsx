...
const CreateTourGuideProfile = () => {
  // Extracting the 'id' from the URL parameters using useParams
  const { id: userId } = useParams();
    console.log(userId);
  // Log the userId to ensure it's being captured
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const userRequest = await axios.post(
        `http://localhost:3000/tourguide/create-profile/${userId}`,
        {
