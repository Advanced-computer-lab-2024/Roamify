...
const ViewAllCreatedItineraries = () => {
    ...
    useEffect(() => {
        const fetchItineraries = async () => {
            try {
                const response = await axios.get("http://localhost:3000/tour-guide/get-itinearies");
                        ...
                        {itineraries.map((itinerary) => (
                            <li key={itinerary._id} className="flex justify-between items-center p-4 border-b border-gray-300">
                                <div>
                                    ...
                                    <p>Activities: {Array.isArray(itinerary.activities) && itinerary.activities.length > 0 ? itinerary.activities.join(", ") : "No activities specified"}</p>
                                    <p>Locations: {Array.isArray(itinerary.locations) && itinerary.locations.length > 0 ? itinerary.locations.join(", ") : "No locations specified"}</p>
                                    <p>Available Dates: {Array.isArray(itinerary.availableDates) && itinerary.availableDates.length > 0 ? itinerary.availableDates.map(date => new Date(date).toLocaleDateString()).join(", ") : "No available dates"}</p>
                                    <p>Language: {itinerary.language || "Language not specified"}</p>
                                    <p>Price: ${itinerary.price || "Price not specified"}</p>
                                    <p>Pick-up Location: {itinerary.pickUpLocation || "Pick-up location not specified"}</p>
                                    <p>Drop-off Location: {itinerary.dropOffLocation || "Drop-off location not specified"}</p>
                                    <p>Accessibility: {itinerary.accessibility ? "Yes" : "No"}</p>
                                    <p>Booked: {itinerary.booked ? "Yes" : "No"}</p>
                                    <p>Preference Tags: {itinerary.preferenceTags && itinerary.preferenceTags.length > 0 ? itinerary.preferenceTags.map(tag => tag.name).join(", ") : "No preference tags available"}</p>
